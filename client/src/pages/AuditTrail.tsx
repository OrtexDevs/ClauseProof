...
export const AuditTrail: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-[#06b6d4] uppercase tracking-wider">Cryptographic Audit Trail</span>
              <span className="text-xs text-[#a8a29e] font-mono">· SHA-256 Chained</span>
            </div>
            <h1 className="text-2xl font-black text-[#1c1917] tracking-tight">Tamper-Evident Action Log</h1>
          </div>
        </div>

        <button
          onClick={loadAuditData}
          className="px-5 py-2.5 rounded-xl text-sm font-bold bg-card hover:bg-card-hover border border-[#D9CFC7] text-[#1c1917] transition-all flex items-center gap-2 shrink-0"
        >
      ...
      <Card glass className={`p-6 border-2 ${verification?.valid ? 'border-emerald-500/40 bg-gradient-to-r from-emerald-50 to-transparent' : 'border-rose-500/40 bg-gradient-to-r from-rose-50 to-transparent'}`}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="text-right font-mono text-xs text-[#78716c]">
            <div>Checked Entries: <span className="text-[#1c1917] font-bold">{verification?.entries_checked || 0}</span></div>
            <div>Algorithm: <span className="text-[#06b6d4] font-bold">SHA-256 Hash Chain</span></div>
          </div>
        </div>
      </Card>
      ...
      <Card glass>
        ...
        {logs.length === 0 ? (
          <div className="text-center py-16 text-[#78716c]">No audit logs recorded for this filing yet.</div>
        ) : (
          <div className="relative pl-6 border-l-2 border-slate-800 space-y-8 ml-3 my-4">
            {logs.map((log) => (
              <div key={log.id} className="relative group">
                {/* Timeline dot */}
                <div className="absolute -left-[31px] top-1.5 w-4 h-4 rounded-full bg-[#EFE9E3] border-2 border-cyan-400 group-hover:scale-125 transition-transform" />

                <div className="p-4 rounded-2xl bg-[#EFE9E3] border border-[#D9CFC7] hover:border-[#D9CFC7] transition-all space-y-2">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-[#1c1917] capitalize">
                        {log.action.replace(/_/g, ' ')}
                      </span>
                      {log.entity_type && (
                        <span className="text-[10px] px-2 py-0.5 rounded bg-[#EFE9E3] text-[#8c7a65] font-mono uppercase">
                          {log.entity_type}
                        </span>
                      )}
                    </div>
                  ...
                  {log.details && Object.keys(log.details).length > 0 && (
                    <div className="text-xs text-[#44403c] bg-[#EFE9E3] p-2.5 rounded-xl font-mono border border-[#D9CFC7]">
                      {JSON.stringify(log.details, null, 2)}
                    </div>
                  )}

                  {/* Hash Footer */}
                  <div className="pt-2 border-t border-[#D9CFC7] flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-[11px] font-mono text-[#a8a29e]">
                    <div className="flex items-center gap-1.5 truncate">
                      <LinkIcon className="w-3.5 h-3.5 text-[#06b6d4] shrink-0" />
                      <span className="text-[#78716c]">Hash:</span>
                      <span className="text-[#0891b2] truncate">{log.content_hash}</span>
                    </div>
                    <div className="truncate text-[#a8a29e]">
